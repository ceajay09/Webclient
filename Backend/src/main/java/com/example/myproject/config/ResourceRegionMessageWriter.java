 ...
 * <p>
 * This class was directly converted from the given
 * {@link org.springframework.http.codec.ResourceHttpMessageWriter}
...
public class ResourceRegionMessageWriter implements HttpMessageWriter<ResourceRegion> {
    ...
    private Mono<Void> writeResource(ResourceRegion resourceRegion,
        ...
        return headersMono.zipWith(mediaTypeMono)
                ...
                .flatMap(resourceType -> zeroCopy(resourceRegion.getResource(), resourceRegion, message)
                        .orElseGet(() -> {

                            Mono<ResourceRegion> input = Mono.just(resourceRegion);
                            DataBufferFactory bufferFactory = message.bufferFactory();
                            Flux<DataBuffer> body = this.resourceRegionEncoder.encode(input, bufferFactory, elementType,
                                    resourceType, hints);
     ...
     *                    value; for annotated controllers, the
     *                    {@link MethodParameter} can be
    ...
    public Mono<Void> write(Publisher<? extends ResourceRegion> inputStream,
        ...
        return inputStreamMono.flatMap(resourceRegion -> {
            ...
            long endPosition = min(startPosition + resourceRegion.getCount() - 1, contentLength - 1); // where zero copy
                                                                                                      // ends relative
                                                                                                      // to start
                                                                                                      // position
